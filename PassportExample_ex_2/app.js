...
router.route('/profile').get(function(req, res) {
	...
	// 인증된 경우, req.user 객체에 사용자 정보 있으며, 인정안된 경우 req.user는 false값임
	...
	if(Array.isArray(req.user)) {
		res.render('profile.ejs', {user: rerq.user[0]._doc});
	} else {
...
passport.deserializeUser(function(user, done) {
	...
	// 사용자 정보 중 id나 email만 있는 경우 사용자 정보 조회 필요 - 여기에서는 user객체 전체를 패스포트에서 관리
